        ...
        if (version <= configVersion)
        {
            ...
            if (buffer->readBool(nullptr))
            {
                ...
                for (uint32_t a = 0; a < count; a++)
                {
                    ...
                    if (LOGS_ENABLED)
                        DEBUG_D("datacenter(%p) %u loaded (hasAuthKey = %d, 0x%" PRIx64 ")", datacenter, datacenter->getDatacenterId(), (int)datacenter->hasPermanentAuthKey(), datacenter->getPermanentAuthKeyId());
                }
            }
        }
        buffer->reuse();
...
void ConnectionsManager::processServerResponse(TLObject *message, int64_t messageId, int32_t messageSeqNo, int64_t messageSalt, Connection *connection, int64_t innerMsgId, int64_t containerMessageId)
{
    ...
    if (LOGS_ENABLED)
        DEBUG_D("process server response %p - %s", message, typeInfo.name());
    ...
    else if (typeInfo == typeid(TL_msg_container))
    {
        ...
        if (LOGS_ENABLED)
            DEBUG_D("received container with %d items", (int32_t)count);
        for (uint32_t a = 0; a < count; a++)
        {
            ...
            {
                if (LOGS_ENABLED)
                    DEBUG_D("inner message %d id 0x%" PRIx64 " already processed", a, innerMessageId);
                continue;
            }
            if (innerMessage->unparsedBody != nullptr)
            {
...
void ConnectionsManager::requestSaltsForDatacenter(Datacenter *datacenter, bool media, bool useTempConnection)
{
    ...
    request->num = 32;
    sendRequest(
        request, [&, datacenter, id, media](TLObject *response, TL_error *error, int32_t networkType, int64_t responseTime)
        {
...
void ConnectionsManager::registerForInternalPushUpdates()
{
    ...
    sendRequest(
        request, [&](TLObject *response, TL_error *error, int32_t networkType, int64_t responseTime)
        {
        if (error == nullptr) {
...
void ConnectionsManager::processRequestQueue(uint32_t connectionTypes, uint32_t dc)
{
    ...
    for (auto iter = requestsQueue.begin(); iter != requestsQueue.end();)
    {
        ...
        if (LOGS_ENABLED)
            DEBUG_D("messageId for token = %d, 0x%" PRIx64, request->requestToken, request->messageId);
            ...
            for (uint32_t b = 0; b < count; b++)
            {
                ...
                if (networkMessage->invokeAfter)
                {
                    ...
                    if (lastSentMessageRpcId != 0 && lastSentMessageRpcId != message->msg_id)
                    {
                        ...
                        {
                            if (LOGS_ENABLED)
                                DEBUG_D("wrap outgoingBody(%p, %s) to TL_invokeAfterMsg, token = %d, after 0x%" PRIx64, message->outgoingBody, typeid(*message->outgoingBody).name(), networkMessage->requestId, request->msg_id);
                        ...
                        {
                            if (LOGS_ENABLED)
                                DEBUG_D("wrap body(%p, %s) to TL_invokeAfterMsg, token = %d, after 0x%" PRIx64, message->body.get(), typeid(*(message->body.get())).name(), networkMessage->requestId, request->msg_id);
...
void ConnectionsManager::updateDcSettings(uint32_t dcNum, bool workaround)
{
    ...
    sendRequest(
        request, [&, workaround](TLObject *response, TL_error *error, int32_t networkType, int64_t responseTime)
        {
...
void ConnectionsManager::moveToDatacenter(uint32_t datacenterId)
{
    ...
    if (currentUserId)
    {
        ...
        sendRequest(
            request, [&, datacenterId](TLObject *response, TL_error *error, int32_t networkType, int64_t responseTime)
            {
            if (error == nullptr) {
